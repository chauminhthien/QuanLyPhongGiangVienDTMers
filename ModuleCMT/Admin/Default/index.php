...
define('SESSION_LOGIN_ADMIN_KEY', 'HUONG_NGHIEP_DTM_LOGGED_ADMIN');
define('SESSION_ACCOUNT_ADMIN_KEY', 'HUONG_NGHIEP_DTM_ACCOUNT_ADMIN_ID');
...
$url_404 		= URL::create([K_URL_DASH, '404']);
...
if (Session::get(SESSION_LOGIN_ADMIN_KEY) === true){
	...
	if (!in_array($currentMudule, $access_module)){
		$next = false;
		$url = URL::create([K_URL_DASH, 'accounts', 'login'], ['return' => $urlCurrent]);
		URL::redirect($url);
	}
}
