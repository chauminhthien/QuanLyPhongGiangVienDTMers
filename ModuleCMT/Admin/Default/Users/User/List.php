...
$limit = 10;
$totalItem = $model->countAllAccount();
if ($totalItem > 0){
    ...
    if ($totalPage > 1){
        ...
        $pmUrl = [K_URL_DASH, 'users'];
        for ($i = $min; $i <= $max; ++$i){
            $pmGet = ($i > 1) ? ['page' => $i] : [];
            $urlI = URL::create($urlSidebar['user_list'], $pmGet);
        ...
        if ($page < $totalPage){
            $urlNext = URL::create($urlSidebar['user_list'], ['page' => $page + 1]);
            $tpl->merge($urlNext, 'paging_btn_next');
            $urlLast = URL::create($urlSidebar['user_list'], ['page' => $totalPage]);
            $tpl->merge($urlLast, 'paging_btn_last');
        }

        if ($page > 1){
            $pmGet = ($page > 2) ? ['page' => $page - 1] : [];
            $urlPrev = URL::create($urlSidebar['user_list'], $pmGet);
    ...
    $listAccount = $model->getAllAccount($options);
    while($row = $model->fetch($listAccount)){
        $row->url_permissions = URL::create([K_URL_DASH, 'users', 'permissions', $row->id]);
        $row->checked = ($row->status == 1) ? 'checked' : '';
        $row->{"type_{$row->type}"} = "selected";
        $tpl->assign($row, 'listAccount');
    }
